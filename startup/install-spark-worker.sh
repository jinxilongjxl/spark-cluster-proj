...
# 步骤5：配置 Spark Worker（关联 Master 节点）
echo "步骤5：配置 Spark Worker"
...
# 步骤6：启动 Spark Worker
echo "步骤6：启动 Spark Worker"
sudo $SPARK_HOME/sbin/start-worker.sh spark://$MASTER_IP:7077
echo "Spark Worker 启动状态："
sudo $SPARK_HOME/sbin/worker-status.sh
